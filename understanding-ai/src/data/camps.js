export const CAMPS = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Aschenbrenner: Situational Awareness (2024)", url: "https://situational-awareness.ai/" }
    ]
  },
  {
    name: "The Doomers",
    ...
    links: [
      { label: "Yudkowsky & Soares: If Anyone Builds It, Everyone Dies (2025)", url: "https://ifanyonebuildsit.com/" },
      ...
      { label: "AI 2027 — Kokotajlo et al. (scenario forecast)", url: "https://ai-2027.com/" },
  ...
  },
  {
    ...
    belief: "AGI may be coming, but it is governable. The right response is not to stop development and not to accelerate it, but to build the institutional, technical, and regulatory infrastructure that lets us steer it. The argument they make against the Doomers is that despair is not a strategy. The argument they make against the Accelerationists is that 'inevitable' is a political choice dressed as a fact.\n\nThe technical alignment work is real and ongoing, though no approach has been proven at scale. Interpretability research — Anthropic's most seriously — attempts to understand what is happening inside a model's internal representations before deployment rather than only testing its behavior. The goal is to find and audit the circuits responsible for specific outputs, the first step toward being able to correct them. Constitutional AI trains a model to critique its own outputs against a set of stated principles rather than only optimizing for human approval ratings; it is an attempt to move from 'pleasing raters' toward something more like values. Scalable oversight — how do you supervise a system that may exceed human performance in the very domain you're evaluating it in? — is the hardest open problem. Paul Christiano's work on debate (AI systems argue, humans adjudicate) and recursive reward modeling are attempts at this. Stuart Russell's CIRL (Cooperative Inverse Reinforcement Learning) proposes building in uncertainty about human values as a structural property of the system, so that deference is architectural rather than trained.\n\nOn the governance side: AI safety institutes in the UK and US conduct frontier model evaluations before deployment. Anthropic's responsible scaling policy commits not to deploy models that exceed certain capability thresholds without demonstrated safety measures at each level — a form of conditional development. International forums on AI safety attempt coordination across governments, with uneven results.\n\nThe internal debates within the safety field matter and are often invisible in public coverage. Technical alignment researchers and governance-focused researchers sometimes have different threat models and disagree about timelines and priorities. There is also a live tension between 'safety before deployment' and 'learn from deployment' — a tension every frontier lab navigates differently, and on which Safety Institutionalists themselves do not agree.",
    links: [
  ...
  },
  {
    ...
    belief: "Real but uneven progress. AGI, if it comes, requires fundamentally new approaches beyond today's autoregressive architectures. Today's systems are useful tools; they are not on a smooth path to general reasoning. This view gets less attention because it has no eschatological drama — no Rapture, no Fall, no urgency. But it is where many serious researchers actually sit, and it deserves more weight than the public discourse gives it.\n\nThe most rigorous recent statement of this position is Narayanan and Kapoor's 'AI as Normal Technology': AI will transform society the way electricity and the internet did — over decades, gated by institutional adoption, regulation, and the slow work of integration, not by raw capability. On this view the control problem is not taming a superintelligence; it is the familiar, unglamorous problem of governing a powerful general-purpose technology. The blind spot worth naming: every prior general-purpose technology diffused slowly because it required physical infrastructure. Software does not, and AI that improves AI may not behave like electricity at all.",
    links: [
      { label: "Narayanan & Kapoor: AI as Normal Technology (2025)", url: "https://knightcolumbia.org/content/ai-as-normal-technology" },
