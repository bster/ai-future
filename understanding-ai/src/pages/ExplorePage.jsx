...
export default function ExplorePage() {
  ...
  return (
    <div>
      ...
      <div role="tablist" aria-label="Explore sections" style={{ display: "flex", gap: 0, marginBottom: "28px", borderBottom: `1px solid ${c.hairline}` }}>
        {TABS.map(t => (
          <button
            ...
            onClick={() => setTab(t.id)}
            style={{ background: "none", color: tab === t.id ? c.primary : c.inkMute, border: "none", borderBottom: tab === t.id ? `2px solid ${c.primary}` : "2px solid transparent", padding: "10px 20px", fontFamily: font, fontSize: "14px", fontWeight: tab === t.id ? 500 : 300, cursor: "pointer", marginBottom: "-1px", letterSpacing: "-0.1px" }}
          >
            {t.label}
          </button>
        ))}
      ...
      {tab === "apps" && (
        <div role="tabpanel" id="panel-apps" aria-labelledby="tab-apps">
          {EXPLORE_APPS.map((app, i) => (
